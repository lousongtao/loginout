...
public class SSOController extends BaseController {
    ...
    public String login(HttpServletRequest request) {
        ...
        String code = RedisUtil
            .get(UpmsRedisKeyPrefix.SYS_UPMS_SERVER_SESSION_ID + serverSessionId);
        // code校验值
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response,
        ...
        String hasCode = RedisUtil.get(UpmsRedisKeyPrefix.SYS_UPMS_SERVER_SESSION_ID + sessionId);
        // code校验值
        if (StringUtils.isBlank(hasCode)) {
            ...
            RedisUtil.lpush(UpmsRedisKeyPrefix.SYS_UPMS_SERVER_SESSION_IDS, sessionId);
            // 默认验证帐号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            RedisUtil.set(UpmsRedisKeyPrefix.SYS_UPMS_SERVER_SESSION_ID + sessionId, code,
                (int) subject.getSession().getTimeout() / 1000);
            // code校验值
            RedisUtil.set(UpmsRedisKeyPrefix.SYS_UPMS_SERVER_CODE + code, code,
                (int) subject.getSession().getTimeout() / 1000);
        }
        //跳转前,redis刷新当前登录的用户信息
        UpmsUser upmsUser = upmsApiService.selectUpmsUserByUsername(username);
        RedisUtil.hset(BaseConstants.SYS_UPMS_USERS_KEY,username, JSON.toJSONString(upmsUser));
        // 回跳登录前地址
        String backurl = request.getParameter("backurl");
        if (StringUtils.isBlank(backurl)) {
            UpmsSystem upmsSystem = upmsSystemService
                .selectUpmsSystemByName(PropertiesFileUtil.getInstance().get("app.name"));
    ...
    public Object code(HttpServletRequest request) {
        ...
        String code = RedisUtil.get(UpmsRedisKeyPrefix.SYS_UPMS_SERVER_CODE + codeParam);
