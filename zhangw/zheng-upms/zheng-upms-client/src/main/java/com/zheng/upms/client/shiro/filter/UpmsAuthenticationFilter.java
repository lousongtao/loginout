...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    private static final Logger LOGGER                      = LoggerFactory
        .getLogger(UpmsAuthenticationFilter.class);
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response,
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("zheng-upms-client")
            .get("zheng.upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request,
        ...
        StringBuffer ssoServerUrl = new StringBuffer(
            PropertiesFileUtil.getInstance("zheng-upms-client").get("zheng.upms.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("zheng-upms-client")
            .get("zheng.upms.type");
        if ("server".equals(upmsType)) {
        ...
        ssoServerUrl.append("/sso/index").append("?").append("appid").append("=")
            .append(PropertiesFileUtil.getInstance("zheng-upms-client").get("zheng.upms.appID"));
        // 回跳地址
        ...
        ssoServerUrl.append("&").append("backurl").append("=")
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        String cacheClientSession = RedisUtil.get(UpmsRedisKeyPrefix.SYS_UPMS_CLIENT_SESSION_ID + session.getId());
        if (StringUtils.isNotBlank(cacheClientSession)) {
            // 更新code有效期
            RedisUtil.set(UpmsRedisKeyPrefix.SYS_UPMS_CLIENT_SESSION_ID + sessionId, cacheClientSession, timeOut);
            Jedis jedis = RedisUtil.getJedis();
            jedis.expire(UpmsRedisKeyPrefix.SYS_UPMS_CLIENT_SESSION_IDS + cacheClientSession, timeOut);
            jedis.close();
            // 移除url中的code参数
            if (null != request.getParameter("code")) {
                String backUrl = RequestParameterUtil
                    .getParameterWithOutCode(WebUtils.toHttp(request));
        ...
        if (StringUtils.isNotBlank(code)) {
            ...
            try {
                StringBuffer ssoServerUrl = new StringBuffer(PropertiesFileUtil
                    .getInstance("zheng-upms-client").get("zheng.upms.sso.server.url"));
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        RedisUtil.set(UpmsRedisKeyPrefix.SYS_UPMS_CLIENT_SESSION_ID + sessionId, code, timeOut);
                        // 保存code对应的局部会话sessionId，方便退出操作
                        RedisUtil.sadd(UpmsRedisKeyPrefix.SYS_UPMS_CLIENT_SESSION_IDS + code, sessionId, timeOut);
                        LOGGER.debug("当前code={}，对应的注册系统个数：{}个", code,
                            RedisUtil.getJedis().scard(UpmsRedisKeyPrefix.SYS_UPMS_CLIENT_SESSION_IDS + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil
                            .getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
